#!/usr/bin/env python3
"""Script to update the api_product_price_prediction function with the notebook approach"""

def main():
    # Read the current file
    with open('/home/kimo/Synder/app.py', 'r') as f:
        content = f.read()

    # Define the old function (we'll use a part of it to locate)
    old_function_start = "def api_product_price_prediction():"
    # Look for the specific return pattern in the function
    old_function_end = "        }), 500"
    
    # Find the start position of the old function
    start_pos = content.find(old_function_start)
    if start_pos == -1:
        print("Could not find the function to replace")
        return

    # Find the end position of the old function
    end_pos = content.find(old_function_end, start_pos)
    if end_pos == -1:
        print("Could not find the end of the function")
        return

    # Include the complete closing part
    end_pos += len(old_function_end)

    # The new function content
    new_function = '''@app.route('/api/product_price_prediction', methods=['POST'])
def api_product_price_prediction():
    ...
    # Extract the part before and after the old function
    before_function = content[:content.rfind('\n', 0, start_pos)]
    after_function = content[end_pos:]

    # Reconstruct the file content with the new function
    new_content = before_function + '\n\n' + new_function + '\n\n' + after_function.lstrip()

    # Write back to the file
    with open('/home/kimo/Synder/app.py', 'w') as f:
        f.write(new_content)
