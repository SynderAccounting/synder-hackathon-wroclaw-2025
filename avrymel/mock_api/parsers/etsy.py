"""Parser for converting Etsy API responses to standardized Order format."""
...
from models import EtsyOrder, Order


def parse_etsy_order(order_json: Dict[str, Any]) -> Order:
    """Parse Etsy API JSON response to standardized Order object.

    Args:
        order_json: JSON dict from Etsy API response

    Returns:
        Standardized Order object with service identifier set to "Etsy"
    """
    ...
    return Order(
        order_id=etsy_order.order_id,
        created_at=etsy_order.created_at,
        updated_at=etsy_order.updated_at,
        currency=etsy_order.currency,
        financial_status=etsy_order.financial_status,
        subtotal=etsy_order.subtotal,
        discount=etsy_order.discount,
        total_tax=etsy_order.total_tax,
        final_price=etsy_order.final_price,
        billing_address=etsy_order.billing_address,
        shipping_address=etsy_order.shipping_address,
        items=etsy_order.items,
        fulfillment=etsy_order.fulfillment,
        customer=etsy_order.customer,
        service="Etsy",
    )
