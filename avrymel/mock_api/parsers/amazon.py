"""Parser for converting Amazon API responses to standardized Order format."""
...
from models import AmazonOrder, Order


def parse_amazon_order(order_json: Dict[str, Any]) -> Order:
    """Parse Amazon API JSON response to standardized Order object.

    Args:
        order_json: JSON dict from Amazon API response

    Returns:
        Standardized Order object with service identifier set to "Amazon"
    """
    ...
    return Order(
        ...
        updated_at=amazon_order.updated_at,
        currency=amazon_order.currency,
        financial_status=amazon_order.financial_status,
        subtotal=amazon_order.subtotal,
        discount=amazon_order.discount,
        total_tax=amazon_order.total_tax,
        final_price=amazon_order.final_price,
        billing_address=amazon_order.billing_address,
        shipping_address=amazon_order.shipping_address,
        items=amazon_order.items,
        fulfillment=amazon_order.fulfillment,
        customer=amazon_order.customer,
        service="Amazon",
    )
